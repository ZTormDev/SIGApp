...
export function parseCurriculumHtml(html: string): CurriculumData {
  ...
  $("tr").each((_: any, row: any) => {
    ...
    const name = (
      $(row).find('input[name="nom_asign"]').val()?.toString().trim() || ""
    )
      .replace(/\s+/g, " ")
      .trim();
    const semester = parseInt(
      $(row).find('input[name="nivel"]').val()?.toString() || "0",
      10,
    );
    const credits = parseInt(
      $(row).find('input[name="credito"]').val()?.toString() || "0",
      10,
    );
    const department =
      $(row).find('input[name="depto"]').val()?.toString().trim() || "";
    const codAsign =
      $(row).find('input[name="cod_asign"]').val()?.toString().trim() || "";
  ...
  $('select[name="carrera"] option[selected], select option[selected]').each(
    (_: any, el: any) => {
      const text = $(el).text().trim();
      if (
        text &&
        text.length > 3 &&
        !text.includes("Concepción") &&
        !text.includes("Diurna")
      ) {
        career = text;
      }
    },
  ...
  const totalSemesters = durationMatch
